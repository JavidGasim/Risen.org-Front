          ...
          {filteredQuests.map((q, i) => {
            ...
            return (
              <div key={q.id} className="qrow slide-up"
                style={{ background: 'rgba(10,13,20,0.85)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: '20px', padding: '24px 32px', display: 'flex', alignItems: 'center', gap: '24px', animationDelay: `${i * 30}ms`, cursor: isLockedPremium ? 'not-allowed' : 'pointer', opacity: isLockedPremium ? 0.8 : 1 }}
                onClick={() => {
                  if (!isLockedPremium) navigate(`/quest/${q.id}`);
                }}>
                ...
                {(() => {
                  ...
                  return (
                    <>
                      <div style={{ width: '48px', height: '48px', borderRadius: '14px', background: isLockedPremium ? 'rgba(245, 158, 11, 0.05)' : (isDone ? 'rgba(16, 185, 129, 0.05)' : 'rgba(99,102,241,0.08)'), border: isLockedPremium ? '1px solid rgba(245, 158, 11, 0.15)' : (isDone ? '1px solid rgba(16, 185, 129, 0.15)' : '1px solid rgba(99,102,241,0.15)'), display: 'flex', alignItems: 'center', justifyContent: 'center', color: isLockedPremium ? '#F59E0B' : (isDone ? '#10B981' : '#818CF8'), fontWeight: 900, fontSize: '1.1rem', flexShrink: 0 }}>
                        {isLockedPremium ? <Lock size={20} /> : (isDone ? <CheckCircle size={20} /> : i + 1)}
                      </div>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '24px', flexShrink: 0 }}>
                        <div style={{ textAlign: 'right' }}>
                          {isLockedPremium ? (
                            <div style={{ background: 'linear-gradient(135deg, rgba(245, 158, 11, 0.2) 0%, rgba(217, 119, 6, 0.2) 100%)', color: '#FCD34D', fontSize: '0.75rem', fontWeight: 800, padding: '6px 14px', borderRadius: '12px', border: '1px solid rgba(245, 158, 11, 0.3)', textTransform: 'uppercase', letterSpacing: '1px', display: 'flex', alignItems: 'center', gap: '6px' }}>
